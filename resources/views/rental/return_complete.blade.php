@extends('layouts.app')

@section('content')
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-md-8">
                <div class="card">
                    <div class="card-header">本の返却</div>

                    <div class="card-body">
                        <div class="row">
                            <h1>返却が完了しました</h1>
                        </div>
                        <form action="/review"method="post">
                            @csrf
                            <div class="form-group">
                                <label for="starSelect">総合評価</label><br/>
                                <button type="button" id="star_1" class="ryp__star__button" data-hook="ryp-star" onclick="starBrighten(1)"><img src="data:image/svg+xml;base64,"></button>
                                <button type="button" id="star_2" class="ryp__star__button" data-hook="ryp-star" onclick="starBrighten(2)"><img src="data:image/svg+xml;base64,"></button>
                                <button type="button" id="star_3" class="ryp__star__button" data-hook="ryp-star" onclick="starBrighten(3)"><img src="data:image/svg+xml;base64,"></button>
                                <button type="button" id="star_4" class="ryp__star__button" data-hook="ryp-star" onclick="starBrighten(4)"><img src="data:image/svg+xml;base64,"></button>
                                <button type="button" id="star_5" class="ryp__star__button" data-hook="ryp-star" onclick="starBrighten(5)"><img src="data:image/svg+xml;base64,"></button>
                                <input type="hidden" name="rank" value="0" id="starSelect">
                            </div>
                            <div class="form-group">
                                <label for="textarea1">レビューを追加</label>
                                <textarea name="Impressions" placeholder="気に入ったこと/気に入らなかったことは何ですか？この本で感じたことはなんですか？？" id="textarea1" class="form-control"></textarea>
                            </div>
                            <div class="form-group">
                                <label for="#">匿名を希望しますか</label>
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="Anonymous_flag" id="radio1b" value="0" checked>
                                    <label class="form-check-label" for="radio1b">希望しない</label>
                                </div>
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="Anonymous_flag" id="radio1a" value="1">
                                    <label class="form-check-label" for="radio1a">希望する</label>
                                </div>
                            </div>
                            <div class="form-group">
                                <button type="submit" class="btn btn-primary mb-2">投稿する</button>
                            </div>
                        </form>
                    </div>
                </div>
            </div>
        </div>
    </div>
@endsection

<script>
    function starBrighten(starNumber){

        for (  var i = 1;  i <= starNumber;  i++  ) {
            $('#star_'+i).html('<img src="data:image/svg+xml;base64," >')
        }
        for (  var i = starNumber+1;  i <= 5;  i++  ) {
            $('#star_'+i).html('<img src="data:image/svg+xml;base64,">')
        }
        $('#starSelect').val(starNumber);

    }
</script>


